...
public class TestEx2 {
	...
	public static void main(String[] args) {
		...
		List<Figura> ret = ListsProcess.filter(lista, f -> f.getArea() > 20);
		printList(ret);
		System.out.println("\nFiguras Filter 2:");
		ret = ListsProcess.filter(lista, f -> f.getPerimetro() < 15);
		printList(ret);
		System.out.println("\nFiguras Filter 3:");
		ret = ListsProcess.filter(lista, f -> f.getPerimetro() < 15 && f.getArea() > 10);
		printList(ret);
		...
		System.out.println("\nEstudante Filter 1:");
		List<Estudante> ret2 = ListsProcess.filter(lista2, e -> e.getNumMec() < 103);
		printList(ret2);
		System.out.println("\nEstudante Filter 2:");
		ret2 = ListsProcess.filter(lista2, e -> e.getClass().getSimpleName().equals("Bolseiro"));
		printList(ret2);
		}
